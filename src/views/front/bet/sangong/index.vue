<template>
  <div class="app-container">
    <el-row :gutter="20">
            ...
            <el-row :gutter="0" style="width:100%">
               <el-col :span="24" :xs="24">
                  <div class="f2">
                    ...
                    <span>฿现金:{{sgSum.sumCashTh || 0}}</span>
                  </div>
               </el-col>
     ...
     <el-table v-loading="loading" class="betBox" height="500px" :data="sgList"  border :row-class-name="status_change"   @selection-change="handleSelectionChange" >
          ...
          <el-table-column label="卡号" align="center" key="card" prop="card"  width="200px">
           ...
           <!-- <el-table-column label="选择结果" align="center" key="type1" prop="type1" width="240px">
               <template slot-scope="scope">
                <el-radio-group @change='DataChange' v-model.number="scope.row.type1" class="niuniuResult">
...
export default {
  ...
  computed:{
    userName(){
     ...
     ...mapState("game", [ "sgList",'sgSum']),
  },
  methods: {
    ...mapMutations('game',["setSgList","setSgSum"]),
    screencast(){
    ...
    getSend(){
      if(this.isSend == true){
        sangongSave({'json':this.betList}).then(res => {
         ...
         sangongSave({'json':arr2}).then(res => {
          this.loading = false;
        })
      }
    },
    ...
    startBet(){
     ...
     this.setSgSum(this.sumdata)
      sangongOpen({'json':param}).then(res => {
